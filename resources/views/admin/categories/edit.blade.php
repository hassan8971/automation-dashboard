...
@section('content')
    <div class="flex justify-between items-center mb-4" dir="rtl">
        <h1 class="text-2xl font-semibold dark:text-white">ویرایش دسته: {{ $category->name }}</h1>
        <a href="{{ route('admin.categories.index') }}" class="px-4 py-2 text-gray-600 rounded-lg hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-dark-hover transition-colors">
            &rarr; بازگشت به لیست
        </a>
    </div>
    ...
    <form id="update-form" action="{{ route('admin.categories.update', $category) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6" dir="rtl">
            ...
            <div class="md:col-span-2 bg-white dark:bg-dark-paper shadow-md rounded-lg p-6 space-y-4 transition-colors">
                <div>
                ...
                <div>
                    ...
                    <input type="text" name="slug" id="slug" value="{{ old('slug', $category->slug) }}" 
                           class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-dark-bg dark:border-gray-600 dark:text-white transition-colors" dir="ltr">
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="5" 
                              class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-dark-bg dark:border-gray-600 dark:text-white transition-colors">{{ old('description', $category->description) }}</textarea>
            ...
            <div class="md:col-span-1 space-y-6">
                ...
                <div class="bg-white dark:bg-dark-paper shadow-md rounded-lg p-6 transition-colors">
                    <label for="image" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">تصویر دسته‌بندی (جدید)</label>
                    ...
                    @if($category->image_path)
                        <div class="mt-4">
                            <p class="text-sm text-gray-500 dark:text-gray-400 mb-2">تصویر فعلی:</p>
                ...
                <div class="bg-white dark:bg-dark-paper shadow-md rounded-lg p-6 flex items-center transition-colors">
                    <input type="checkbox" name="is_visible" id="is_visible" value="1" 
                           {{ old('is_visible', $category->is_visible) ? 'checked' : '' }}
                           class="h-4 w-4 text-blue-600 border-gray-300 rounded ml-2 focus:ring-blue-500 dark:bg-dark-bg dark:border-gray-600 dark:checked:bg-blue-600">
    ...
    <div class="mt-6 flex justify-between" dir="rtl">
        ...
        <button type="submit" form="update-form" class="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
            ذخیره تغییرات
        </button>
    </div>
